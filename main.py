...
async def komutlar(ctx):
    ...
    embed.add_field(
        ...
        value=(
            ...
            "• `.ara [oyuncu adı]` - Sunucuda oyuncu araması yapar.\n"
...
async def oyuncu_ara(ctx, *, aranan_isim: str = None):
    ...
    for member in ctx.guild.members:
        ...
        if aranan_isim.lower() in nick.lower():
    ...
    if not bulunan_oyuncu:
        embed = discord.Embed(
            title="❌ SONUÇ BULUNAMADI",
            description=f"Sunucuda **{aranan_isim}** adında herhangi bir oyuncu bulunamadı.",
            color=discord.Color.from_str("#E74C3C")
        )
        embed.set_footer(text="Arion League")
        await ctx.send(embed=embed)
    ...
    embed = discord.Embed(
        title="✅ İŞLEM BAŞARILI — OYUNCU BULUNDU",
        description=f"Aradığınız futbolcu sunucumuzda tespit edildi!",
        color=discord.Color.from_str("#2ECC71")
    )
    embed.add_field(name="👤 Kullanıcı", value=bulunan_oyuncu["member"].mention, inline=False)
    embed.add_field(name="💰 Piyasa Değeri", value=f"`{bulunan_oyuncu['deger_str']}`", inline=False)
